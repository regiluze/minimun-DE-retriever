...
with describe('Minimun devops engineer retriever'):
    ...
    with context('when processing the input data'):
        ...
        with context('when there is more than one best data center to place DM'):
            with it('returns the data center with more amount of servers'):
                data_centers = [{'name': DATA_CENTER_NAME, 'servers': 61 },
                                {'name': ANOTHER_DATA_CENTER_NAME, 'servers': 62}]

                _, DM_data_center_name = self.retriever.process(DM_capacity=20, DE_capacity=10, data_centers=data_centers)
